...
export type PlotMarker = { id: string; name: string; lng: number; lat: number };
...
export function FarmSatelliteMap({
  ...
  // Add `VITE_MAPBOX_TOKEN=your_token_here` to `frontend/.env`, then restart Vite to load it.
  ...
  useEffect(() => {
    ...
    plots.forEach((plot) => {
      ...
      const popup = new mapboxgl.Popup({ offset: 14 }).setText(plot.name);
    ...
    <div className="relative w-full" style={{ height: 360 }}>
      ...
      {!mapboxToken && (
        <div className="absolute inset-0 flex items-center justify-center rounded-2xl bg-[#F9FAFB] text-[#6B7280] text-sm px-6 text-center">
          Mapbox token missing. Add it to your env file to enable the satellite map.
        </div>
      )}
    </div>
  );
